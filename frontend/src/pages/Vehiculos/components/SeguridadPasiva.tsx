...
const FIELD_LABELS: Record<string, string> = {
  cinturonesDeSeguridad: 'Cinturones de seguridad',
  airbags: 'Airbags',
  chasisCarrocería: 'Chasis y carrocería',
  cristalesParabrisas: 'Cristales parabrisas',
  reposacabezas: 'Reposacabezas',
  kitCarretera: 'Kit de carretera',
};

const SeguridadPasiva: React.FC = () => {
  const [formData, setFormData] = useState<FormData>(
    SEGURIDAD_PASIVA_FIELDS.reduce((acc, field) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const camposIncompletos = SEGURIDAD_PASIVA_FIELDS.filter(field => !formData[field]);
  ...
  return (
    <div className='flex justify-center items-center min-h-screen bg-gray-100 py-10'>
      <div className="bg-white p-8 rounded-lg shadow-lg w-full sm:w-96 overflow-y-auto max-h-screen">
       <h1 className='text-3xl font-bold text-center text-gray-800 mb-6'>Seguridad Pasiva</h1>

      <form onSubmit={handleSubmit}>
        {SEGURIDAD_PASIVA_FIELDS.map((field) => (
          <div key={field}>
...
export default SeguridadPasiva;
